<div class='bg-white rounded-xl px-5 py-3 flex items-center justify-between gap-5'>
    ...
    <div class='flex text-sm items-center gap-2'>
        <p class='text-center break-all'>"{{ $quote->getTranslation('body', app()->getLocale()) }}"</p>
        <p>
            |
        </p>
        <a href='{{ route('movie', $quote->movie->id) }}'
            class='break-all min-w-60 text-center'>{{ $quote->movie->name }}</a>
    </div>
