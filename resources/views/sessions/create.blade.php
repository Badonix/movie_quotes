<x-layout>
    ...
    <main class='h-screen w-full flex flex-col justify-center items-center px-10'>
        <div class='bg-neutral-400 rounded-xl p-12 w-full max-w-lg md:w-2/3 lg:w-1/2'>
            ...
            <form method='POST' action='{{ route('login.store') }}' class='mt-5 flex flex-col gap-5'>
                @csrf
                <div>
                    <x-input form_name='username' name='username'>
                        @error('username')
                            <p class='text-sm text-red-600 absolute -bottom-3'>{{ $message }}</p>
                        @enderror
                    </x-input>
                </div>
                <div>
                    <x-input form_name='password' name='password' type='password'>
                        ...
                        @enderror
                        @error('wrong')
                            <p class='text-red-600 absolute -bottom-4'>{{ $message }}</p>
                        @enderror
                    </x-input>
                </div>
                <button
                    class='py-2 px-4 border border-gray-600 bg-slate-50 text-gray-700 font-semibold rounded-sm mt-6 hover:bg-slate-100'
